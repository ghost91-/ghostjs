...
(async () => {
  const { devDependencies } = JSON.parse(readFileSync(`./package.json`).toString());
  ...
  const results = await Promise.all(
    projects
      ...
      .map(async (project) => {
        ...
        const { dependencies, peerDependencies } = JSON.parse(readFileSync(`${projectPath}/package.json`).toString());

        const missing = argv.missing
          ? await getMissingDependencies(project.name, projectPath, dependencies, peerDependencies, argv.verbose)
          : [];

        const discrepancies = argv.discrepancies ? getDiscrepancies(project.name, dependencies, devDependencies) : [];
  ...
  results.forEach(({ name, missing, discrepancies }) => {
    ...
    if (missing.length > 0) {
      ...
      console.log(
        `⚠️  ${chalk.bold.inverse(` Missing `)}\n${missing
          .sort()
          .map((p) => `   ${devDependencies[p] ? `${p}@${devDependencies[p]}` : p}`)
          .join(`\n`)}\n`,
      );
    }
    ...
    if (discrepancies.length > 0) {
      ...
      console.log(`⛔  ${chalk.bold.inverse(` Discrepancies `)}\n${discrepancies.map((d) => `   ${d}`).join(`\n`)}\n`);
    }
  });
