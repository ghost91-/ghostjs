...
import { findEarliestAncestralDirpath, findExpandedFilepath, isMatch, relativePath, resolvePath } from './path';
  ...
  for (const target of targets) {
    if (isMatch(workspace, srcPath, target.watchPatterns)) {
      for (const destination of target.dest) {
        const copyTarget: ICopyTargetItem = generateCopyTarget(workspace, srcPath, destination, target);
    ...
    if (matchedPaths.length) {
      for (const matchedPath of matchedPaths) {
        ...
        for (const destination of destinations) {
          const copyTarget: ICopyTargetItem = generateCopyTarget(workspace, matchedPath, destination, target);
